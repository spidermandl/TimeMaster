...
public class FrameActivity extends FragmentActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		tabHost.addTab(tabHost.newTabSpec("generation")
				.setIndicator(generationTab)
				.setContent(R.id.generation_fragment));
		tabHost.addTab(tabHost.newTabSpec("year").setIndicator(yearTab)
				.setContent(R.id.year_fragment));
		tabHost.addTab(tabHost.newTabSpec("month").setIndicator(monthTab)
				.setContent(R.id.month_fragment));
		tabHost.addTab(tabHost.newTabSpec("week").setIndicator(weekTab)
				.setContent(R.id.week_fragment));
		tabHost.addTab(tabHost.newTabSpec("date").setIndicator(dateTab)
				.setContent(R.id.date_fragment));
		tabHost.addTab(tabHost.newTabSpec("list").setIndicator(listTab)
				.setContent(R.id.issue_list_fragment));
		tabHost.addTab(tabHost.newTabSpec("new").setIndicator(newTab)
				.setContent(R.id.new_issue_fragment));
	...
	// AndroidFragment androidFragment = (GenerationFragment)
	// fm.findFragmentById(arg0)findFragmentByTag("android");
	// AppleFragment appleFragment = (AppleFragment)
	// fm.findFragmentByTag("apple");
	...
	public void showNext(int containerID, Class<Fragment> fragmentName,
			int fragmentID) {
	...
	public void onConfigurationChanged(Configuration newConfig) {
		...
		TimeMasterApplication.getInstance()
				.setScreenMode(newConfig.orientation);
		tabHost.invalidate();
