...
public class WorldTimeCitysViewGroup extends BasicViewGroup{
	
	
	public WorldTimeCitysViewGroup(Context context) {
		super(context);
		// TODO Auto-generated constructor stub
	}
	public WorldTimeCitysViewGroup(Context context, AttributeSet attrs) {
		super(context, attrs);
		init();
	}
	
	public WorldTimeCitysViewGroup(Context context, AttributeSet attrs, int defStyle) {
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		setMeasuredDimension(4*unit_width+4*gap,height);
		System.out.println(height);
		System.out.println(4*unit_width+4*gap);
	}
	@Override
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		// TODO Auto-generated method stub
		current_margin_top=unit_width+2*gap;
		System.out.println(current_margin_top);
		int num=this.getChildCount();
		for(int i=0;i<num;i++){
			View view=this.getChildAt(i);
			LayoutStyleableInterface styleable=(LayoutStyleableInterface)view;
